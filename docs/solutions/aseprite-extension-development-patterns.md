...
20. **Dialog:modify on labels causes ghost artifacts** — Changing label text via `dlg:modify{ id, text }` leaves visual remnants of the old text. Rebuild the entire dialog (close + reopen in a loop) instead of modifying labels in-place.
21. **Use canvas widgets for dynamic lists** — Labels grow unbounded and cause dialog resize jitter. Use a fixed-height canvas with `onpaint`/`onwheel` for lists that change. See `dialog_utils.scrollableList()`.
22. **Dialog sizing must be explicit** — Dialogs with dynamic content must use fixed dimensions. See `docs/window-sizes.md` for the registry. Any new dialog type must be registered before implementation.
23. **Palette not inherited by Sprite()** — `Sprite(w, h, colorMode)` gets a default palette. Capture with `Palette(spr.palettes[1])` before blueprint operations, apply with `setPalette()` after creation. Wrap `Palette()` in pcall — may not exist in all builds.
24. **Canvas drag-and-drop: differentiate click vs drag** — Store `dragStartY` on mousedown. In mouseup, if distance < 4px, treat as click (open/create). Otherwise complete the drag-drop. Update `dragMouseY` in mousemove for visual feedback.
25. **Rebuild-loop pattern for modal dialogs** — Instead of modifying widgets in a blocking dialog, use a `while true` loop that builds a fresh dialog each iteration. Actions close the dialog, the loop reads `dlg.data`, applies the action, and rebuilds. See `blueprint_editor.lua` for examples.
